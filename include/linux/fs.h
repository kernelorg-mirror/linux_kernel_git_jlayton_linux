...
/**
 * inode_ctime_nsec_peek - peek at (but don't query) the ctime tv_nsec field
 ...
 */
static inline long inode_ctime_nsec_peek(const struct inode *inode)
{
	return inode->__i_ctime.tv_nsec & ~I_CTIME_QUERIED;
}

/**
 * inode_ctime_peek - peek at (but don't query) the ctime
 * @inode: inode to fetch the ctime from
 *
...
static inline struct timespec64 inode_ctime_peek(const struct inode *inode)
{
	...
	ctime.tv_nsec = inode_ctime_nsec_peek(inode);
