...
#define NFSD_A_SERVER_SOCK_USERSPACE_RPCBIND	2
#define NFSD_A_SERVER_SOCK_RPCBIND	3	/* per-registration nest */
#define NFSD_A_SOCK_ADDR		1	/* inside the addr nest */
#define NFSD_A_SOCK_TRANSPORT_NAME	2	/* inside the addr nest */
#define NFSD_A_RPCBIND_PROGRAM		1	/* inside the rpcbind nest */
#define NFSD_A_RPCBIND_VERSION		2	/* inside the rpcbind nest */
#define NFSD_A_RPCBIND_FLAGS		3	/* inside the rpcbind nest */
#define NFSD_RPCBIND_FLAGS_NO_UDP	1
 ...
 * passes flags of 0, so pmap_register is true in svc_setup_socket(). A
 * failure there no longer takes the listener down, but with nothing
 * listening every attempt waits out the local rpcbind timeout, once per
 * program and version. The abstract AF_LOCAL name the kernel tries first is
 * per-netns (unix_find_abstract() takes a struct net), so answer it here and
 * stay out of the host's rpcbind.
 *
