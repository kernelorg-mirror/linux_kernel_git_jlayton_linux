...
int hfsplus_cat_read_inode(struct inode *inode, struct hfs_find_data *fd)
{
	...
	if (type == HFSPLUS_FOLDER) {
		...
		inode_set_ctime(inode,
				hfsp_mt2ut(folder->attribute_mod_date).tv_sec,
				hfsp_mt2ut(folder->attribute_mod_date).tv_nsec);
		...
		inode_set_ctime(inode,
				hfsp_mt2ut(file->attribute_mod_date).tv_sec,
				hfsp_mt2ut(file->attribute_mod_date).tv_nsec);
