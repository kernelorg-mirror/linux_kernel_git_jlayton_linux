						       ...
						       work_func_t worker)
{
	...
	if (subreq) {
		...
		case NETFS_DIRECT_BV:
			subreq->iter = wreq->direct_iter;
			iov_iter_advance(&subreq->iter, subreq->start - wreq->start);
			break;
		case NETFS_BUFFER:
			iov_iter_xarray(&subreq->iter, WRITE, &wreq->buffer,
		...
		case NETFS_COPY_ENC_BOUNCE:
			...
			break;
		default:
			kdebug("Invalid buffering form %u", wreq->buffering);
			BUG();
		}
				       ...
				       bool was_async)
{
	...
	if (iov_iter_count(&subreq->iter) != subreq->len - subreq->transferred)
		pr_warn("R=%08x[%u] ITER POST-MISMATCH %zx != %zx-%zx %x\n",
			wreq->debug_id, subreq->debug_index,
			iov_iter_count(&subreq->iter), subreq->len,
			subreq->transferred, subreq->iter.iter_type);
...
static void netfs_write_to_cache_op(struct netfs_io_subrequest *subreq)
{
	...
	cres->ops->write(cres, subreq->start, &subreq->iter,
...
static void netfs_writeback_worker(struct work_struct *work)
{
...
{
	_enter("R=%x %llx-%llx %lx-%lx f=%lx",
	       wreq->debug_id, wreq->start, wreq->start + wreq->len - 1,
	       wreq->first, wreq->last, wreq->flags);

	trace_netfs_wback(wreq);
	if (wreq->len == 0) {
		pr_err("Zero-sized write [R=%x]\n", wreq->debug_id);
		netfs_put_request(wreq, false, netfs_rreq_trace_put_zero_len);
		return -EIO;
	}
