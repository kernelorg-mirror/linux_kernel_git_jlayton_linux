			...
			struct dentry *dentry, umode_t mode, bool excl)
{
	...
	dir->i_mtime = inode_get_ctime(dir) = inode_get_ctime(inode);
...
static int ubifs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	lock_2_inodes(dir, inode);
	inode_get_ctime(inode) = current_time(dir);
	drop_nlink(inode);
	dir->i_size -= sz_change;
	dir_ui->ui_size = dir->i_size;
	dir->i_mtime = inode_get_ctime(dir) = inode_get_ctime(inode);
		     ...
		     unsigned int flags)
{
	...
	inode_set_ctime(old_inode, time.tv_sec, time.tv_nsec);
	...
	old_dir->i_mtime = inode_set_ctime(old_dir, time.tv_sec, time.tv_nsec);
	new_dir->i_mtime = inode_set_ctime(new_dir, time.tv_sec, time.tv_nsec);
	...
	if (unlink) {
		...
		inode_set_ctime(new_inode, time.tv_sec, time.tv_nsec);
	} else {
			...
			struct inode *new_dir, struct dentry *new_dentry)
{
	...
	time = current_time(old_dir);
	inode_set_ctime(fst_inode, time.tv_sec, time.tv_nsec);
	inode_set_ctime(snd_inode, time.tv_sec, time.tv_nsec);
	old_dir->i_mtime = inode_set_ctime(old_dir, time.tv_sec, time.tv_nsec);
	new_dir->i_mtime = inode_set_ctime(new_dir, time.tv_sec, time.tv_nsec);
