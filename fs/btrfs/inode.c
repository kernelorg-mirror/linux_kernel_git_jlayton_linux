			      ...
			      struct dentry *new_dentry)
{
	...
	struct timespec64 ctime = inode_set_ctime_current(old_inode);
	...
	inode_set_ctime(old_dir, ctime.tv_sec, ctime.tv_nsec);
	new_dir->i_mtime = ctime;
	inode_set_ctime(new_dir, ctime.tv_sec, ctime.tv_nsec);
	inode_set_ctime(new_inode, ctime.tv_sec, ctime.tv_nsec);
			...
			unsigned int flags)
{
	...
	inode_get_ctime(new_dir) = old_dir->i_mtime;
	inode_get_ctime(old_inode) = old_dir->i_mtime;
