...
struct fscache_cookie *fscache_hash_cookie(struct fscache_cookie *candidate)
{
	...
	hlist_bl_for_each_entry(cursor, p, h, hash_link) {
		if (fscache_compare_cookie(candidate, cursor) == 0) {
			pr_notice("Potential collision c=%08x old: c=%08x",
				  candidate->debug_id, cursor->debug_id);
	...
	if (wait_for) {
		wait_var_event(&wait_for->stage,
			       READ_ONCE(wait_for->stage) == FSCACHE_COOKIE_STAGE_DROPPED);
...
void __fscache_relinquish_cookie(struct fscache_cookie *cookie, bool retire)
{
	...
	/* Make sure those who are checking the state under look have looked at
	...
	if (cookie->stage == FSCACHE_COOKIE_STAGE_QUIESCENT)
