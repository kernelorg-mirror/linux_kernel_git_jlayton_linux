			...
			struct dentry *dentry, umode_t mode, bool excl)
{
	...
	dir_i->i_mtime = inode_get_ctime(dir_i) = ITIME(je32_to_cpu(ri->ctime));
...
static int jffs2_unlink(struct inode *dir_i, struct dentry *dentry)
{
	...
	if (!ret)
		dir_i->i_mtime = inode_get_ctime(dir_i) = ITIME(now);
	return ret;
}
/***********************************************************************/
			  ...
			  struct dentry *dentry, const char *target)
{
	...
	*/
	...
	dir_i->i_mtime = inode_get_ctime(dir_i) = ITIME(je32_to_cpu(rd->mctime));
			 ...
			 unsigned int flags)
{
	...
	*/
	...
	if (ret) {
		...
		new_dir_i->i_mtime = inode_get_ctime(new_dir_i) = ITIME(now);
		return ret;
	}
	...
	new_dir_i->i_mtime = inode_get_ctime(new_dir_i) = old_dir_i->i_mtime = inode_get_ctime(old_dir_i) = ITIME(now);
