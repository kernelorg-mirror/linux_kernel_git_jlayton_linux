...
cifs_fattr_to_inode(struct inode *inode, struct cifs_fattr *fattr)
{
	...
	inode_set_ctime(inode, fattr->cf_ctime.tv_sec,
			fattr->cf_ctime.tv_nsec);
...
int cifs_rmdir(struct inode *inode, struct dentry *direntry)
{
	...
	d_inode(direntry)->i_ctime = inode->i_mtime = inode_set_ctime_current(inode);
...
unlink_target:
	...
	inode_get_ctime(source_dir) = source_dir->i_mtime = inode_get_ctime(target_dir) =
