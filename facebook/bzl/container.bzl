load(":flavors.td.bzl", "ARCH_AARCH64", "ARCH_X86_64", "ARCHITECTURES", "ARCH_FLAVORS")
...
def host_container_image(arch):
    if native.host_info().arch.is_aarch64:
        return _IMAGE_MAP[ARCH_AARCH64]
    return _IMAGE_MAP[ARCH_X86_64]

#
# We should just forward to either host_container_image or
# target_container_image based on build_info.use_aarch64_emul,
# but x86_64 emulation via binfmt_misc sucks, so we can't.
#
def default_container_image(arch):
    # binfmt_misc for x86_64 is flakey, so skip it
    if native.host_info().arch.is_aarch64:
        return _IMAGE_MAP[ARCH_AARCH64]
    if native.read_config("build_info", "use_aarch64_emul", "0") == "1":
        return target_container_image(arch)
    # our happy path is cross-compiling
    return _IMAGE_MAP[ARCH_X86_64]
...
def container_genrule(name, arch, cmd, pre_cmd = None, bind_ro = None, bind_rw = None, overlay_ro = None, overlay_rw = None, tmpfs = None, cacheable = False, image_override=None):
    ...
    if image_override:
        ...
        image = "$(location {})".format(default_container_image(arch))
