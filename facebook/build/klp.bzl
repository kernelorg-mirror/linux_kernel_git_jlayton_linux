load("//facebook/config:flavors.td.bzl", "ARCH_X86_64", "ARCHITECTURE_TO_KERNEL_ARCH")
load("//facebook/build:container.bzl", "container_genrule", "target_container_image")
...
def klp():
    ...
    following configuration parameters:
      ...
      - extraver: used by lto kernel to specify extra version string.
    """
    ...
    extraver = native.read_config("klp", "extraver", None)
    if not extraver:
      extraver = ""
    ...
    container_genrule(
        ...
        """.format(train_data_args = train_data_args, extraver = extraver, compiler_args = compiler_args),
        bind_ro=bind_ros,
        bind_rw=bind_rws,
        cacheable=False,
